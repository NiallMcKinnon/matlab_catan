function [matrixMap, canBuildRoad, canBuildSettlement, victoryPoints] = buildSettlementTurn1(victoryPoints, matrixMap, canBuildRoad, canBuildSettlement, xNumberMap, yNumberMap, teamNumber, oppositeTeam, color) %<SM:PDF> %<SM:PDF_PARAM> %<SM:PDF_RETURN>
...
while flag == true %<SM:WHILE>
    ...
    if canBuildSettlement(rPoint, cPoint) == teamNumber || canBuildSettlement(rPoint, cPoint) == 3 || isnan(canBuildSettlement(rPoint, cPoint)) || canBuildSettlement(rPoint, cPoint) == oppositeTeam %<SM:IF> %<SM:BOP>
